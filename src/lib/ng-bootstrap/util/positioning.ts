...
export class Positioning {
  ...
  public positionElements(
    ...
    let availablePlacements = this.getAvailablePlacements(hostElement, targetElement);
    ...
    for (let { item, index } of toItemIndexes(placementVals)) {
      ...
      if ((availablePlacements.find(val => val === item) != null) || (placementVals.length === index + 1)) {
        ...
        const pos = this._positionElements(hostElement, targetElement, item, appendToBody);
        topVal = pos.top;
        leftVal = pos.left;
        break;
      }
  ...
  private _positionElements(hostElement: HTMLElement, targetElement: HTMLElement, placement: string, appendToBody?: boolean):
    ClientRect {
